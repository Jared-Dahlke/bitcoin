    ...
    : QDialog(parent, GUIUtil::dialog_flags)
{
    ...
    connect(m_copy_button, &QPushButton::clicked, this, [this] { GUIUtil::setClipboard(m_descriptors.join("\n")); });
...
void CreateMultisigWalletDialog::validate()
{
    ...
    if (walletName().isEmpty()) {
        ...
        std::string error;
        const auto parsed_desc{Parse(receive_desc, keys, error, /*require_checksum=*/false)};
        if (!parsed_desc) {
            ...
            for (const std::string& fragment : fragments) {
                ...
                const auto frag_desc{Parse("wpkh(" + fragment + "/0/*)", frag_keys, frag_error, /*require_checksum=*/false)};
                if (frag_desc && frag_desc->Expand(/*pos=*/0, frag_keys, frag_scripts, frag_provider) && !frag_scripts.empty()) {
                    unique_child_scripts.insert(frag_scripts.at(0));
                }
            }
            FlatSigningProvider provider;
            std::vector<CScript> scripts;
            if (!parsed_desc->Expand(/*pos=*/0, keys, scripts, provider) || scripts.empty()) {
                ...
                receive_desc += "#" + GetDescriptorChecksum(receive_desc);
                change_desc += "#" + GetDescriptorChecksum(change_desc);
                m_descriptors << QString::fromStdString(receive_desc) << QString::fromStdString(change_desc);
                m_first_address = QString::fromStdString(EncodeDestination(dest));
                m_descriptor_preview->setPlainText(m_descriptors.join("\n"));
